...
export async function getPollution(request: FastifyRequest, reply: FastifyReply) {
    ...
    const data = await PollutionProvider.getPollution(lat, lon);

    if (!data) {
        return reply.code(503).send({ error: 'Pollution data unavailable', availability: false });
    }
